...
<template>
  <UContainer class="py-4 space-y-4">
    ...
    <UCard v-if="generatedFilters" class="mt-4">
      ...
      <div v-else class="h-64 flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-700">
      ...
      <div class="mt-4 text-sm text-gray-600 dark:text-gray-400 space-y-1">
        <p><span class="font-medium">Selected Accounts:</span> {{ generatedFilters.accounts.length ? generatedFilters.accounts.join(', ') : 'All' }}</p>
        <p><span class="font-medium">Currency:</span> {{ generatedFilters.currency }}</p>
        <p><span class="font-medium">Categories:</span> {{ generatedFilters.categories.length ? generatedFilters.categories.length + ' selected' : 'All' }}</p>
        <p><span class="font-medium">Projects:</span> {{ generatedFilters.projects.length ? generatedFilters.projects.length + ' selected' : 'All' }}</p>
      </div>
    </UCard>
