...
<template>
    ...
    <ContextMenu :id="tx.id" resource="transaction">
      ...
      >
        <div class="flex items-center">
          ...
          <DateView
            :date="tx.date"
            class="w-16 cursor-pointer hover:text-primary-500"
        ...
        >
          ...
          <div class="text-right shrink-0 min-w-[80px]">
            <p
              :class="textColorByAmount(tx.amount)"
  ...
  <!-- Assert Item -->
  <div
    v-else-if="assertItem"
    :id="assertItem.data.id"
    class="relative py-2 my-2 group hover:bg-gray-50 dark:hover:bg-gray-800/20 rounded"
  >
    <div
      class="absolute left-0 right-0 h-px transition-colors duration-300 top-1/2 -z-10"
      :class="
        Math.abs(assertItem.diff) < 0.005
    ...
    >
      ...
      <span class="font-mono"
        >Expected: {{ formatAmount(assertItem.data.value) }}</span
      >
      <span
        v-if="Math.abs(assertItem.diff) >= 0.005"
        class="ml-2 flex items-center gap-1"
      >
        <span class="text-gray-400">|</span>
        <span>Actual: {{ formatAmount(assertItem.actual) }}</span>
        <span class="font-bold"
          >(Diff: {{ formatAmount(assertItem.diff) }})</span
        >
      </span>
      <UIcon
        v-if="Math.abs(assertItem.diff) < 0.005"
    ...
    >
      <UButton
        ...
        color="neutral"
        variant="ghost"
        @click="emit('edit-assert', assertItem.data)"
        title="Edit Assert"
      />
      <UButton
        ...
        color="error"
        variant="ghost"
        @click="emit('delete-assert', assertItem.data.id)"
        title="Delete Assert"
      />
    </div>
  </div>
