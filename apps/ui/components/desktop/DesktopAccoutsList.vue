...
<template>
  <main class="mx-auto max-w-7xl">
    <header class="flex items-center justify-between border-b border-white/5 px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
      ...
      <NuxtLink class="text-sm font-semibold leading-6 text-indigo-400" to="/account/new?edit=1">New account
      </NuxtLink>
    </header>

    <!-- Deployment list -->
    <ul class="divide-y divide-white/5" role="list">
      <li v-for="account in accountStore.sortedAccounts.filter((acc) =>
          ...
          <NuxtLink :to="`/account/${account.id}`"
            ...
            <div class="min-w-0 flex-auto">
              ...
              <div class="mt-3 flex items-center gap-x-2.5 text-xs leading-5 text-gray-400">
                ...
                <svg class="h-0.5 w-0.5 flex-none fill-gray-300" viewBox="0 0 2 2">
